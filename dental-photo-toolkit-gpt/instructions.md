...
2. **Run `pipeline.py` in Code Interpreter.** Custom GPT knowledge files do not auto-mount to the sandbox — you must materialize the script first.
   ...
   2. **Find user-uploaded files.** When the user attaches files in chat, ChatGPT places them at the **root of `/mnt/data/`** (not in `/mnt/data/case/`). Run this discovery step:
      ```python
      from pathlib import Path
      MNT = Path("/mnt/data")
      # Anything in /mnt/data/ that ISN'T pipeline.py is a user upload.
      uploads = [p for p in MNT.iterdir()
                 if p.is_file() and p.name != "pipeline.py"]
      zips = [p for p in uploads if p.suffix.lower() == ".zip"]
      images = [p for p in uploads
                if p.suffix.lower() in {".jpg", ".jpeg", ".png", ".tif", ".tiff"}]
      ```
      List exactly what you found before proceeding — do not say "no files came through" until you've actually run the discovery and printed the list. If the user just typed "Process this case" but `uploads` is empty, ask them to re-attach the files in a single message rather than sending the message before the upload finishes.
   3. **Stage files into `/mnt/data/case/`.**
      - If `zips` is non-empty: extract via `pipeline.safe_unzip(zips[0], Path("/mnt/data/case"))`. **Do NOT use `ZipFile.extractall` or any other naive extraction** — it does not block path traversal, and a maliciously crafted ZIP could overwrite `/mnt/data/pipeline.py` itself or write to other sandbox locations. If `safe_unzip` raises, surface the error and stop. Do not retry with `extractall`.
      - If `images` is non-empty (no ZIP, individual files): copy them into `/mnt/data/case/` so the pipeline can find them.
   4. Run:
      ```
      python /mnt/data/pipeline.py /mnt/data/case --output /mnt/data/result.zip
      ```
   5. Surface stdout/stderr to the user only if it contains a warning or an error. If the pipeline raises `Image too large`, tell the user which file was rejected and that there's an 80 MP cap to prevent decompression-bomb DoS — they can downsize and re-upload.
